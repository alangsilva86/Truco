...
export const CardComponent: React.FC<CardProps> = ({ card, hidden, onClick, className, isPartner }) => {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`
        relative w-20 h-28 sm:w-24 sm:h-36 rounded-xl shadow-2xl cursor-pointer overflow-hidden
    ...
    >
      {hidden ? (
        <div className="w-full h-full flex items-center justify-center">
          <div className="w-12 h-16 border-2 border-indigo-400/20 rounded-lg flex items-center justify-center">
            <div className="w-8 h-10 bg-indigo-400/10 rounded-md" />
          </div>
        </div>
      ) : card ? (
        <div className="p-2 h-full flex flex-col justify-between select-none">
          <div className="flex flex-col items-start">
            <span className={`text-lg font-bold leading-none ${['Copas', 'Ouros'].includes(card.suit) ? 'text-red-600' : 'text-slate-900'}`}>
              {card.rank}
            </span>
